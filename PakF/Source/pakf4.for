...
C=======================================================================
       ...
       IMPLICIT DOUBLE PRECISION(A-H,O-Z)
C       COMMON /NUMNPT/ NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET
       DIMENSION NODES(5),NEL(NDIM+1,*),IPM(8,3),IBRP(3),IBRM(3)
C
...
C       CALL KOORD(CK,H,RST0)       
	...
	AK=2.D0*AMI/GUSM
      ...
      AX=0.D0
      AY=0.D0
      IF (DABS(ALFAX).GT.1.D-10) AX=1.D0/DTANH(ALFAX)-1.D0/ALFAX
      IF (DABS(ALFAY).GT.1.D-10) AY=1.D0/DTANH(ALFAY)-1.D0/ALFAY
...
C       AZ=HUGHES(ALFAZ)
      ...
      DO I=1,NDIM
        PP=AK*(HV1*ZVHX(I)+HV2*ZVHY(I))/VV
        HH(I)=HH(I)+PP
      ENDDO
...
C=======================================================================
C==========================================================================
       SUBROUTINE INTER1(CK,V1,V2,V3,HH,PJ,NDIM,AMI)
       IMPLICIT DOUBLE PRECISION(A-H,O-Z)
C       COMMON /VISKOZ/ AMI,INDAMI
C       COMMON /KONST/ GUSM(1),CC,AKT
C       COMMON /NUMNPT/ NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET
       ...
       DIMENSION RST0(3),EX(3),EY(3),EZ(3),XJJ(3,3)
	 DIMENSION LE(3),ME(3)
...
C	AK=2.D0*AMI/GUSM(1)
	AK=2.D0/AMI

	hx=dabs(ck(1,1)-ck(8,1))
	hy=dabs(ck(1,2)-ck(8,2))
	hz=dabs(ck(1,2)-ck(8,3))
      ALFAX=hv1*HX/AK
      ALFAY=hv2*HY/AK
      ALFAZ=hv3*HZ/AK
...
c      AKK=((AX*UX*HX+AY*UY*HY+AZ*UZ*HZ)/2.D0)
      AKK=((AX*hv1*HX+AY*hv2*HY+AZ*hv3*HZ)/2.D0)


C ovde menjano oko UPWIND, Oct. 21, 2006
      DO I=1,NDIM
c        PP=AKK*(HV1*PJ(1,I)+HV2*PJ(2,I)+HV3*PJ(3,I))/VV
        PP=AK*(HV1*PJ(1,I)+HV2*PJ(2,I)+HV3*PJ(3,I))/VV
        HH(I)=HH(I)+PP
      ENDDO
      ...
      lambda=sqrt((ck(1,1)-ck(8,1))**2+(ck(1,2)-ck(8,2))**2+
     &(ck(1,3)-ck(8,3))**2)

      DO I=1,NDIM
        PP=VV*lambda/sqrt(15.d00)
        HH(I)=HH(I)+PP
      ENDDO
...
C      COMMON /NDESUK/ NDES,IDPRIT,IFORM
      ...
      DIMENSION TT1(*),ID(6,*),NEL(NDIM+1,*),CORD(3,*),PRIT(IDPRIT,*)
...
C==========================================================================
      ...
      DIMENSION NEL(NDIM+1,*),TANG(*),ANORM(*),CORD(3,*)
...
C      COMMON /NDESUK/ NDES,IDPRIT,IFORM
      ...
      DIMENSION GNODE(2,6,*),ID(6,*),NEL(NDIM+1,*),CORD(3,*)
...
C==========================================================================
      ...
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
	DIMENSION GNODE(2,6,*),ID(6,*),ANORM(*),TANG(*),V(3),VNEW(*)
...
C ......................................................................
C
      DIMENSION NEL(NDIM+1,*),NN(4)
      DIMENSION PRITP(IDPRIT,*),CORD(3,*)
      CHARACTER*250NASLOV
      DIMENSION GNODE(2,6,*),ID(6,*),FSP(6)
...
C=======================================================================
      ...
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      DIMENSION NEL(NDIM+1,*),CORD(3,*),NN(4)
C
...
C
CE Subroutine EXPAND is used for special calculation for
...
C==========================================================================
      ...
      DIMENSION CORD(3,*),CCORD(3,*),TT1(*),ID(6,*)
      DIMENSION XXLM(3)
C
...
C==========================================================================
     ...
     &NDIM,KKORAK,VVREME,NETIP,GNODE,ID)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
     
	COMMON /RESTART/ IRESTA,RESTVR
     
      DIMENSION CCORD(3,*),CORD(3,*),TT1(*),SPSIL(NETIP,*),GNODE(2,7,*)
      DIMENSION NEL(NDIM+1,*),INDEL(*),ID(7,*)
...
C just for Andre example, Aug 16, 2007
C         GNODE(1,5,I)=37.D0
...
15     CALL CLEAR(TT1,NEQF)
      IF (IRESTA.EQ.1) THEN
	  CALL REINIT(GNODE,NPT,RESTVR)
	ENDIF
...
C==========================================================================
     ...
     &IDPRIT,NEL,NET,NETIP,CORD,IIZLAZ)
      ...
      DIMENSION INDEL(*),ID(7,*),NEL(NDIM+1,*)
      DIMENSION GNODE(2,7,*),TT1(*),N(21),PRIT(IDPRIT,*),P(21),CORD(3,*)

C
CE Subroutine RETPRI is used for calculation pressure at all nodes per element
C

C INITIALISATION PRES MATRIX
...
C          GNODE(2,5,NODE)=GNODE(2,5,NODE)+PP/(INDEL(NODE)*1.D0)
       ENDDO
      ENDDO

      ELSEIF(PENALT.LT.1.D0) THEN
...
C      ENDIF        
C      IF(NDIM.EQ.9.AND.NETIP.EQ.2) THEN
...
C==========================================================================
     ...
     &IDPRIT,NEL,NET,NETIP)
      ...
      DIMENSION INDEL(*),ID(7,*),NEL(NDIM,*)
      DIMENSION GNODE(2,7,*),TT1(*),N(21),PRIT(IDPRIT,*),P(21)
C
...
C==========================================================================
     ...
     &IIZLAZ,NUMZAD,NPT,CORD)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
       DIMENSION GNODE(2,7,*),ZADVRE(*),TABF(2,NTABFT,*),CORD(3,*)
...
C	  T=5.D0
C        PI=4.D0*DATAN(1.D0)
C     	  FK1=0.643564D0*0.5D0*DSIN(2.D0*PI*VVREME/T)
C        IF(NZAD(1,I).GT.(0.75*NPT)) FK1=VSR1
C====================================
c        r=dsqrt(cord(1,nzad(1,i))**2+cord(2,nzad(1,i))**2)
c	  v=2.d0*(1.d0-(r/10.0d-4)**2)
c	  v3=ZADVRE(I)*cos(PI/3.d0)
c	  v1=ZADVRE(I)*sin(PI/3.d0)
c        write(iizlaz,'(3i5,f10.7)')nzad(1,i),3,1,v3
c        write(iizlaz,'(3i5,f10.7)')nzad(1,i),1,1,-v1
...
C==========================================================================
     ...
     &IIZLAZ,IUPWIN,XG,WGT,NREF,IBRGT,PRES,INDEL,NBREL,NZAD,ZADVRE)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      DIMENSION NEL(NDIM+1,*),ID(7,*),INDEL(*),NZAD(3,*)
...
C==========================================================================
C==========================================================================
      SUBROUTINE SSTRE2(NEL,ID,NPARAM,TT21,H,HP,X,Y,R,S,W,ZVHX,ZVHY,
     &PRES,INDEL,NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET,AMI,INDAMI,PENALT,
     &INDAX,AKT,GUSM,IUPWIN,NBREL,IIZLAZ,IBRGT)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)

      COMMON /TURB/ ITURB
      DIMENSION NEL(NDIM+1,*),ID(7,*),INDEL(*)
      DIMENSION TT21(*),H(*),HP(*),X(*),Y(*)
      DIMENSION R(3,*),S(3,*),W(3,*),ZVHX(*),ZVHY(*),PRES(3,*)
	DIMENSION NID(4)
	
C
CE Subroutine SSTRE2 is used for calculation shear stresses for 2D problem
	...
	DO I=1,4
	NID(I)=0
      IF (ID(1,NEL(I,NBREL)).EQ.0.AND.ID(2,NEL(I,NBREL)).EQ.0) NID(I)=1
	ENDDO

      DO 225 I=1,IBRGT-1

      IF (NID(1)*NID(4).EQ.1) THEN 
       IF (ITURB.EQ.0) THEN
     ...
     &AKT,GUSM,IUPWIN,NBREL,IIZLAZ)
        CALL FPRES(PRES,AMI,N1,N4,IIZLAZ,INDEL)
        ELSE
        CALL FNTERPT(1.D0,S(IBRGT-1,I),NPARAM,TT21,H,HP,ZVHX,ZVHY,HV2,
     &HV3,ZVXT,ZVYT,DETJ,DETJS,X,Y,FS2,FS3,ZVXV2,ZVYV3,ZVYV2,ZVXV3,TAU,
     &NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET,AMI,INDAMI,PENALT,PRESS,INDAX,
     &AKT,GUSM,IUPWIN,FSK1,FSOMEGA1,HKT,HOM,ZVXK,ZVYK,ZVXOM,ZVYOM,NBREL,
     &IIZLAZ,GNODE,NEL,CORD,ID,TT210,PRES)
        CALL FPRES(PRES,AMI,N1,N4,IIZLAZ,INDEL)
        ENDIF
      ENDIF 

      IF (NID(2)*NID(3).EQ.1) THEN 
       IF (ITURB.EQ.0) THEN
      CALL FNTERP(-1.D0,S(IBRGT-1,I),NPARAM,TT21,H,HP,ZVHX,ZVHY,HV2,
     &HV3,ZVXT,ZVYT,DETJ,DETJS,X,Y,FS2,FS3,ZVXV2,ZVYV3,ZVYV2,ZVXV3,TAU,
     &NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET,AMI,INDAMI,PENALT,PRESS,INDAX,
     &AKT,GUSM,IUPWIN,NBREL,IIZLAZ)
        CALL FPRES(PRES,AMI,N2,N3,IIZLAZ,INDEL)
        ELSE
        CALL FNTERPT(1.D0,S(IBRGT-1,I),NPARAM,TT21,H,HP,ZVHX,ZVHY,HV2,
     &HV3,ZVXT,ZVYT,DETJ,DETJS,X,Y,FS2,FS3,ZVXV2,ZVYV3,ZVYV2,ZVXV3,TAU,
     &NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET,AMI,INDAMI,PENALT,PRESS,INDAX,
     ...
     &IIZLAZ,GNODE,NEL,CORD,ID,TT210,PRES)
      ...
      IF (NID(1)*NID(2).EQ.1) THEN 
       IF (ITURB.EQ.0) THEN
      CALL FNTERP(R(IBRGT-1,I),1.D0,NPARAM,TT21,H,HP,ZVHX,ZVHY,HV2,
     &HV3,ZVXT,ZVYT,DETJ,DETJS,X,Y,FS2,FS3,ZVXV2,ZVYV3,ZVYV2,ZVXV3,TAU,
     &NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET,AMI,INDAMI,PENALT,PRESS,INDAX,
     &AKT,GUSM,IUPWIN,NBREL,IIZLAZ)
        CALL FPRES(PRES,AMI,N1,N2,IIZLAZ,INDEL)
        ELSE
        CALL FNTERPT(1.D0,S(IBRGT-1,I),NPARAM,TT21,H,HP,ZVHX,ZVHY,HV2,
     &HV3,ZVXT,ZVYT,DETJ,DETJS,X,Y,FS2,FS3,ZVXV2,ZVYV3,ZVYV2,ZVXV3,TAU,
     &NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET,AMI,INDAMI,PENALT,PRESS,INDAX,
     ...
     &IIZLAZ,GNODE,NEL,CORD,ID,TT210,PRES)
      ...
      IF (NID(3)*NID(4).EQ.1) THEN 
      IF (ITURB.EQ.0) THEN
      CALL FNTERP(R(IBRGT-1,I),-1.D0,NPARAM,TT21,H,HP,ZVHX,ZVHY,HV2,
     &HV3,ZVXT,ZVYT,DETJ,DETJS,X,Y,FS2,FS3,ZVXV2,ZVYV3,ZVYV2,ZVXV3,TAU,
     &NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET,AMI,INDAMI,PENALT,PRESS,INDAX,
     &AKT,GUSM,IUPWIN,NBREL,IIZLAZ)
        CALL FPRES(PRES,AMI,N3,N4,IIZLAZ,INDEL)
        ELSE
        CALL FNTERPT(1.D0,S(IBRGT-1,I),NPARAM,TT21,H,HP,ZVHX,ZVHY,HV2,
     &HV3,ZVXT,ZVYT,DETJ,DETJS,X,Y,FS2,FS3,ZVXV2,ZVYV3,ZVYV2,ZVXV3,TAU,
     &NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET,AMI,INDAMI,PENALT,PRESS,INDAX,
...
C==========================================================================
C==========================================================================
      SUBROUTINE FPRES(PRES,AMI,N1,N2,IIZLAZ,INDEL)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
...
C      WRITE(IIZLAZ,*)'SHEAR STRESS= ',SHEAR(1),SHEAR(2)

      PRES(1,N1)=PRES(1,N1)-AMI*SHEAR(1)*0.5D0/INDEL(N1)
      PRES(2,N1)=PRES(2,N1)-AMI*SHEAR(2)*0.5D0/INDEL(N1)
       
      PRES(1,N2)=PRES(1,N2)-AMI*SHEAR(1)*0.5D0/INDEL(N2)
      PRES(2,N2)=PRES(2,N2)-AMI*SHEAR(2)*0.5D0/INDEL(N2)
...
C=======================================================================
C=======================================================================
      SUBROUTINE SPRIZL(PRIT,NET,NETIP,IIZLAZ,IBRGT,IDPRIT,PENALT)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
     ...
     &,PERIO1,TOL,NUMST,MESH,NUMALV,NBSTAC,NPRVEL,MOVEW,IAKIRA
      ...
      IF (IAKIRA.NE.0) RETURN

      IF (PENALT.GT.0.D0) THEN

      WRITE(IIZLAZ,*)'   ELEMENT   ','  PRESSURE  '
      DO 10 I=1,NET
      IF (NETIP.EQ.2) THEN
...
C=======================================================================
C==========================================================================
      SUBROUTINE REINIT(GNODE,NPT,VVREME)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      
      DIMENSION GNODE(2,7,*)
      III=88
      OPEN(III,FILE='INITIAL.UNV')
...
c          GNODE(J,3,N)=VZ
 ...
 100   FORMAT(I10)
 200   FORMAT(3(1PE13.5))
      END
C=======================================================================
C==========================================================================
      SUBROUTINE INITI0(TT1,ID,NPT,CORD,GNODE)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)

      DIMENSION TT1(*),ID(7,*),CORD(3,*),GNODE(2,7,*)
      X0=0.28D0
...
C=======================================================================
C==========================================================================
      SUBROUTINE BURGMV(CCORD,DX,NPT)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      ...
      DO I=1+3,NPT-3
C
C      CCORD(1,I)=CCORD(1,I)+0.005*DX
C      CCORD(1,I)=CCORD(1,I)-0.005*DX
      ENDDO
...
C=======================================================================
      ...
      ANI=1.D0
      E1=DEXP(-ANI*PI**2*T)
      E2=DEXP(-4.D0*ANI*PI**2*T)
      G1=A1*E1*DSIN(PI*X)
      G2=2*A2*E2*DSIN(2.D0*PI*X)
      G=2.D0*PI*ANI*(G1+G2)

      D1=A1*E1*DCOS(PI*X)
      D2=A2*E2*DCOS(2*PI*X)
      D=A0+D1+D2
...
C=======================================================================
C==========================================================================
      SUBROUTINE MOVING(NPT,CCORD,TIME,CORD,KKORAK,VMESH,DX,ND)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
	DIMENSION CCORD(3,*),VMESH(3,*),CORD(3,*)
       
       
       NSTEP=100
c       DX=CORD(1,4)-CORD(1,1)
       DD=0.75*DX
       D=DD/(NSTEP/2)

C SUBROUTINE FOR MOVING NODES-LAGRANGIAN METHOD
C       DO 10 NODE=7,NPT-3,12
C       IF(KKORAK.LE.NSTEP/2) N=1
C       IF(KKORAK.GT.NSTEP/2) N=-1
C       CCORD(1,NODE)=CCORD(1,NODE)+D*N
C       CCORD(1,NODE+1)=CCORD(1,NODE+1)+D*N
C       CCORD(1,NODE+2)=CCORD(1,NODE+2)+D*N
C       VMESH(1,NODE)=(D*N)/TIME
C       VMESH(1,NODE+1)=(D*N)/TIME
C       VMESH(1,NODE+2)=(D*N)/TIME
C 10   CONTINUE

       DO 10 NODE=ND+1,NPT-ND
       IF(KKORAK.LE.NSTEP/2) N=1
       IF(KKORAK.GT.NSTEP/2) N=-1
       CCORD(1,NODE)=CCORD(1,NODE)+D*N
       VMESH(1,NODE)=(D*N)/TIME
 10   CONTINUE

      END
C==========================================================================
C==========================================================================
      SUBROUTINE INITI1(TT1,ID,NPT,CORD,GNODE)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      ...
      END
C==========================================================================
      SUBROUTINE BURGERPRINT(CCORD,GNODE,NPT,T,IIZLAZ)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)

      DIMENSION CCORD(3,*),GNODE(2,6,*)
c Printing for Burger's non-viscous example
c=======================================================================
      do i=1,npt,3
	  x=ccord(1,i)
	  write(iizlaz,'(3(1pe13.5))') x,gnode(2,1,i)
	enddo 
      return
c=======================================================================
c Printing for Burger's viscous example
c=======================================================================
c      do i=1,npt,3
        i=21
	  x=ccord(1,i)
c	  write(iizlaz,'(3(1pe13.5))') x,gnode(2,1,i),ANAL(X,T)
	  write(iizlaz,'(3(1pe13.5))') t,gnode(2,1,i),ANAL(X,T)
c	enddo 

	END
c=======================================================================
C==========================================================================
      SUBROUTINE INDELSSTRES(NEL,INDEL,NET,NPT,NDIM,ID)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
...
C==========================================================================
C==========================================================================
      SUBROUTINE OPTBAND(ID,CORD,NPT,IOSA,CCORD)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
...
C
CE Subroutine OPTBAND is used for optimal ID packing
C
	...
	ENDDO
       ...
       DO I=1,NPT-1
	  DO J=I+1,NPT
         IF (CCORD(IOSA,J).GT.CCORD(IOSA,I)) THEN
          A=CCORD(IOSA,J) 	     
          CCORD(IOSA,J)=CCORD(IOSA,I)
          CCORD(IOSA,I)=A
	    IA=ID(7,J)
	    ID(7,J)=ID(7,I)
	    ID(7,I)=IA
	...
	K=0 
      DO I=1,NPT
	 IPOS=ID(7,I)
	 IF(ID(IOSA,IPOS).NE.0) THEN
	   K=K+1
	   ID(IOSA,IPOS)=K
	 ENDIF
	CCORD(IOSA,I)=CORD(IOSA,I)
	ENDDO
