...
CE at ID matrix
C
      DIMENSION ID(7,*),IS(3),IE(3),IS3(2),IE3(2)
...
C
     ...
     *WRITE(IIZLAZ,2051)
      ...
      RETURN
C----------------------------------------------------------------------
 2050 FORMAT('1'/////////30X,'P R O G R A M'/
     1 10X,'      FOR FINITE ELEMENT ANALYSIS OF LAMINAR AND '/ 
     1 17X,'TURBULENT FLUID FLOW WITH HEAT TRANSFER')
 2051 FORMAT('1'/////////33X,'P R O G R A M'/
     1 10X,'ZA ANALIZU LAMINARNOG I TURBULENTNOG STRUJANJA '/
     1 19X,'NESTISLJIVOG VISKOZNOG FLUIDA SA PROVODJENJEM TOPLOTE'/
 ...
 2115 FORMAT(//
     ...
     1 30X,'UNIVERSITY OF KRAGUJEVAC'/
     1 30X,'SESTRE JANJICA 6'/
     1 30X,'34000 KRAGUJEVAC'/
     1 30X,'REPUBLIC OF SERBIA'/
     1 30X,'E-mail: fica@kg.ac.rs'/
     1 30X,'        dziga@kg.ac.rs'/
     1 30X,'Year 2020'/
     1 30X,'                      '////)
       END
C==========================================================================
C=======================================================================
     ...
     1PRITP,CCORD,CORD,NDIM,NETIP,PENALT,IDPRIT,ISRPS,SPSIL,PRES,ID)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
...
C
	...
	DIMENSION ID(7,*)
      ...
      CALL FILUNV(II,KOR,CORD,ID,NP,NEL,NDIM,NET,1,NETIP)
...
CS    BROJ PODATAKA = 6
     ...
     1WRITE(II,6000) KOR
      ...
      DO 10 I=1,NP
         ...
         DO 20 J=1,NETIP
            FSP(J) = 0.0D0
c            IF(ID(J,I).ne.0) fsp(j)=1.d0 
C            K = ID(J,I)
C               FSP(J)=RTH(K)
               FSP(J)=GNODE(2,J,I)
   20    CONTINUE
C         WRITE(II,5200) (FSP(J),J=1,NETIP)
C Samo privremeno za primer Electrical Epicardial Ablation
c         WRITE(II,5201) gnode(2,6,i),0.e0
         WRITE(II,5201) (FSP(J),J=1,NETIP)
   ...
   32    CONTINUE
        ...
        DO 38 K=1,NBROJ
         IF (NDIM.EQ.9) THEN 
            ...
            if (k.gt.4)gnode (2,4,nel(k,i))=prit1(k)
         ELSE 
...
c      WRITE(II,3006) 2,1,1,123,2,NETIP
      WRITE(II,3006) 1,1,KOR,KOR
      WRITE(II,5202) VREME
      DO  I=1,NP
            WRITE(II,5000) I
c            WRITE(II,5200) (PRES(J,I)/1.d1,J=1,3)
            WRITE(II,5200) (PRES(J,I),J=1,3)
C Samo privremeno za primer Electrical Epicardial Ablation
C            WRITE(II,5200) GNODE(2,5,I),0.D0,0.D0

C SAMO PRIVREMENO ZA STAMPANJE SILA OD FLUIDA:
c            WRITE(II,5200) (SPSIL(J,I),J=1,NETIP)
	ENDDO
      WRITE(II,5100) IND1
...
C
C  FOR PRINTING wall forces AT NODES
C
C NUMBER '13' IS FOR wall forces AT NODES
      ...
      WRITE(II,7009)
      WRITE(II,2001) KOR
      WRITE(II,3006) 2,1,1,123,2,1
      WRITE(II,3006) 1,1,KOR,KOR
      WRITE(II,5202) VREME
      DO  I=1,NP
            WRITE(II,5000) I
c            WRITE(II,5200) GNODE(2,4,I)
            WRITE(II,5200) (SPSIL(J,I),J=1,NETIP)
	ENDDO
      WRITE(II,5100) IND1
...
c           FSP(J)=SPSIL(J,I)
        ENDDO
C SAMO PRIVREMENO ZA STAMPANJE SILA OD FLUIDA:
c            WRITE(II,5200) (SPSIL(J,I),J=1,NETIP),0.d0,0.d0,0.d0
c          fsp(3)=gnode(2,5,i)
...
C-----------------------------------------------------------------------
 ...
 7009 FORMAT(' WALL FORCES AT NODES')
...
C          IF (R.EQ.1. .OR. R.EQ.-1.) THEN
  ...
  30  CONTINUE
      ...
      IF (NPARAM.EQ.1) THEN
c       FS2=AMI*(ELX*VXX+ELY*VXY)-ELX*PRESS
c       FS3=AMI*(ELX*VYX+ELY*VYY)-ELY*PRESS
       FS2=AMI*(ELX*VXX+ELY*VXY)-ELX*PRESS
       FS3=AMI*(ELX*VYX+ELY*VYY)-ELY*PRESS
C      ZXPRESS=0.D0
C      ZYPRESS=0.D0
C	 DO I=1,4
C	  ZXPRESS=ZXPRESS+ZVHX(I)*TT21(2*NDIM+I)
C	  ZYPRESS=ZYPRESS+ZVHY(I)*TT21(2*NDIM+I)
C	 ENDDO
C      FS2=ELX*ZXPRESS
C       FS3=ELY*ZYPRESS
C       FS2=-ELX*PRESS
C       FS3=-ELY*PRESS
C================================================================
C for pressure boundary condition:
C       FS2=elx
C       FS3=ely
C================================================================
...
C       TAU=-AKT*(ELX*TX+ELY*TY)
       FS2=AKT*(ELX*TX+ELY*TY)
      ENDIF
...
C======================================================================
C==========================================================================
      SUBROUTINE SHEARR(AN,TT21,VTANG,NDIM,ETX,ETY) 
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
       DIMENSION TT21(*),VTANG(2,*),AN(*),V(2),T(2)
 
C
CE Subroutine SHEAR2 is used for calculation shear stresses for 2D 
C
       DO I=1,NDIM
         V(1)=TT21(I)          
         V(2)=TT21(I+NDIM)          
...
C     END
C==========================================================================
        SUBROUTINE OUTFLU(IF,LIDF,LCORDF,NPF)
        IMPLICIT DOUBLE PRECISION(A-H,O-Z)
        DIMENSION IF(*)
...
C==========================================================================
C==========================================================================
      SUBROUTINE FILUNV(II,KOR,CORD,ID,NPT,NEL,NDIM,NET,NGE,NETIP)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
	...
	STRING='PAKF'//INTSTR(KOR)//'.'//'UNV'
        OPEN(II,FILE=STRING)
        IF (KOR.GT.1) CLOSE(II-NPRINT)
       CALL TGRAFC(CORD,NPT,II)
       CALL TGRBCF(CORD,NPT,II,ID)
       CALL TGRAFF(NEL,NDIM,NET,NGE,II,NETIP,NDIM)
