C=========================================================================
      SUBROUTINE tetra_seg_elem(GNODE,ALEVO,DESNO,SILE,NEL,
     1ID,NGPSIL,MAXA,CORD,SKEF,PRIT,SPSIL,IBRGT,
     &ITFMAX,TABF,GUSM,CC,AKT,NETIP,IIZLAZ,AMI,INDAMI,BETA,TETAO,FB2,
     &FB3,NUMZAD,NPT,NDIMM,MAXSIL,JEDN,NWK,NET,NDES,IDPRIT,IFORM,NPER,
     &NTABFT,PENALT,PRES,NSTAC,VVREME,TIME,INDEL,ZADVRE,NZAD,IUPWIN,
     &VMESH,IALE,AF,NJUTRA,ISYMMS,ITER,IPASS,METOD,NUMPASS,id1,nbrel,
     &NDIM,nell)
...
C
      ...
      DIMENSION NELL(*),NEL(NDIMM+1,*),ID(6,*),NGPSIL(8,*),MAXA(*)
...
c      if (ndim.eq.4) ndimp=4
      if (ndim.eq.4) ndimp=11
...
C=========================================================================
      DO 130 KLM=1,NDIM
      CK(KLM,1)=CORD(1,NELL(KLM))
      CK(KLM,2)=CORD(2,NELL(KLM))
      CK(KLM,3)=CORD(3,NELL(KLM))
	tez=tez+0.125d0*CORD(3,NELL(KLM))
      LM2(KLM)=ID(1,NELL(KLM))
      LM2(KLM+NDIM)=ID(2,NELL(KLM))
      LM2(KLM+2*NDIM)=ID(3,NELL(KLM))
      if (KLM.LE.ndimp) then
C	if (nel(ndim+1,nbrel).ne.0) then
C	   LM2(klm+3*NDIM)=ID(4,NEL(ndim+1,NBREL))
C      else
	   LM2(KLM+3*NDIM)=ID(4,NELL(KLM))
C	endif
	endif
c      IF (KLM.LE.1) LM2(KLM+3*NDIM)=ID(4,NEL(KLM,NBREL))
      LM2(KLM+3*NDIM+ndimp)=ID(5,NELL(KLM))
 130  CONTINUE
C=======================================================================
...
C=========================================================================
      DO 140 KLM=1,NDIM
      DO 135 NR=1,3
      TT21(KLM+(NR-1)*NDIM)=GNODE(2,NR,NELL(KLM))
      TT210(KLM+(NR-1)*NDIM)=GNODE(1,NR,NELL(KLM))
 135  CONTINUE
...
C	else
        TT21(KLM+3*NDIM)=GNODE(2,4,NELL(KLM))
        TT210(KLM+3*NDIM)=GNODE(1,4,NELL(KLM))
C	endif
      ENDIF
        TT21(KLM+3*NDIM+ndimp)=GNODE(2,5,NELL(KLM))
        TT210(KLM+3*NDIM+ndimp)=GNODE(1,5,NELL(KLM))
...
C
      IF(NELL(NR).EQ.NZAD(1,NZDT)) THEN
...
c      WT=WGT(JR)*WGT(JS)*WGT(JT)
	...
	if (ndim.eq.4) then
     ...
     &hv2,hv3,HXU,HYU,HZU,HXV,HYV,HZV,HXW,HYW,HZW,NELL,ndimp,hpv)
...
C     &AKV3P(K1,N)*(1.D0/AKIIZ(K1,K1))*AKV3P(K1,K)
	...
	ENDIF
      ...
      do i=1,4
	 do j=2,4
	   if (nell(i).eq.ngpsil(j,jbrps)) then
	     nn=nn+1
	     nizel(nn)=nell(i)
	   endif
	 enddo
...
c      CALL Tetra_SSTRES(NEL,NDIM,ID1,CK,PJ,H,HP,TT21,AMI,ISRPS,NUMZAD,
c     &IIZLAZ,IUPWIN,XG,WGT,NREF,IBRGT,PRES,INDEL,NBREL,NZAD,ZADVRE,CORD)
