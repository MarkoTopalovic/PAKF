...
C=======================================================================
     ...
     &KOR,VREME,INDSC,INDAX)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      COMMON /BROJFK/ INDFOR,NULAZ
      COMMON /CDEBUG/ IDEBUG

      CHARACTER*250NASLOV
      DIMENSION GNODE(2,6,*),NEL(NDIM+1,*)
      DIMENSION CCORD(3,*)
      DIMENSION A(*)    
      REAL A
...
C      CALL MAXMEM(LMAX,NTOT,IIZLAZ,KOR,NWK)

      CALL CLEARR(A(LMAX0),LMAX-LMAX0+1)
      
     
      CALL STREM2(GNODE,A(LID),CCORD,NEL,A(LMAXA),A(LTT1),A(LALEVO),
     &A(LSKE),A(LSILE),A(LSKEF),NPT,NET,NDIM,NDES,NEQ,IIZLAZ,NWK,INDAX)

      CALL STRUNV(GNODE,A(LID),A(LTT1),59,NASLOV,KOR,VREME,NPT,INDSC)
...
C=======================================================================
     ...
     &SKEF,NPT,NET,NDIM,NDES,NEQ,IIZLAZ,NWK,INDAX)

      IMPLICIT DOUBLE PRECISION(A-H,O-Z)

      DIMENSION TT1(*),ALEVO(*),SKE(*),SILE(*),CORD(3,*),SKEF(NDES,*)
      DIMENSION GNODE(2,6,*)
      DIMENSION NEL(NDIM+1,*),ID(3,*),MAXA(*)
...
c      IF (NBREL.EQ.3240) goto 400
c      IF (NBREL.EQ.4760) goto 400
C      DO I=1,NDIM
C       DO J=1,NDIM
C        A11(I,J)=0.D0
C        ADX(I,J)=0.D0
...
C 305  CONTINUE
	  ...
	  VY=GNODE(2,2,NODE)
        TT21(JJ+1)=VY*RADIUS
        TT21(JJ+2)=-VX*RADIUS
c	if (mod(node,16).eq.1) tt21(jj)=-7.7d0
C	if (mod(node,16).eq.1) tt21(jj)=-4.8d-4
C	if (mod(node,16).eq.1) tt21(jj)=-4.6d-1
       ENDDO

       DO 320 I=1,NDES
        DO 320 J=1,NDES
        F36(I)=F36(I)-SKEF(I,J)*TT21(J)
 320   CONTINUE
...
C     IF (INDAX.EQ.1) RADIUS=CORD(1,NODE)
C==========================================
C      JX=ID(2,NODE)       
C       IF (JX.NE.0) SILE(JX)=GNODE(2,2,NODE)*1.0D35*RADIUS
C       ALEVO(MAXA(JX))=1.0D35
C      JY=ID(3,NODE)       
C       IF (JY.NE.0) SILE(JY)=-GNODE(2,1,NODE)*1.0D35*RADIUS
C        ALEVO(MAXA(JY))=1.0D35
C 405   CONTINUE
...
C      CALL RESENF(ALEVO,SILE,MAXA,NEQ,NWK,1)
      CALL RESENF(ALEVO,TT1,MAXA,NEQ,NWK,1)
...
C       ID(3,I)=0
      ...
      ENDDO
       ...
       ID(1,npt)=1
C samo za primer za Akiru -MAE (Z=0 ravan)
C      do i=1,npt,16
C       id(1,i)=1
C      enddo
C      do i=16,npt,16
...
C=======================================================================
C=======================================================================
      SUBROUTINE STRUNV(GNODE,ID,TT1,II,NASLOV,KOR,VREME,NPT,INDSC)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      DIMENSION ID(3,*)
      DIMENSION GNODE(2,6,*),TT1(*)
