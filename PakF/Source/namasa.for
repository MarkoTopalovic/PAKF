C=========================================================================
C=========================================================================
      SUBROUTINE NAMASA(GNODE,NEL,ID,CORD,SKEF,
     ...
     &PRES,VMESH,IALE,AF,INDEL,IPRESS,IPASS,PRITIS,AMASA,ID1)
...
C NBREL is counter of elements
 100  CONTINUE
	...
	CALL MAXATF1(MAXA,MHT,ID,NEL,NET,NDIM,JEDN1,NWK1,5,NDIM,4)
...
C	  AKU_1(K,K)=AKU_1(K,K)+DABS(AKU1(K,N))
	ENDDO
C        VMESH(1,NODE)=VMESH(1,NODE)+AK
	ENDDO

C       CALL WRRF(AKU_1,16,'AKU_1=',IIZLAZ)
C      DO K=1,NDIM
C        NODE=NEL(K,NBREL)
C	  AMASA(ID1,NODE)=AMASA(ID1,NODE)+AKU_1(K,K)
C	ENDDO

      CALL ADDSTF(ALEVO,SILE,DESNO,AKU1,F31,MAXA,LM2,NDIM,1)
...
C=======================================================================
 400  CONTINUE
c	  AMASA(ID1,NODE)=DABS(AMASA(ID1,NODE))
c	ENDDO
	 ...
	 JJ=I
	IF (JJ.EQ.1) THEN
	 AMASA(ID1,I)=AMASA(ID1,I)+DABS(ALEVO(1))
	GOTO 500
	ENDIF
	   K=MAXA(JJ)
	   AMASA(ID1,I)=AMASA(ID1,I)+DABS(ALEVO(K))
	DO K=MAXA(JJ)-1,MAXA(JJ-1)+1,-1
	   AMASA(ID1,I)=AMASA(ID1,I)+DABS(ALEVO(K))
	   AMASA(ID1,I)=AMASA(ID1,I)+DABS(DESNO(K))
	ENDDO
C	WRITE(IIZLAZ,*) 'AMASA(ID1,JJ)= ', I,JJ,AMASA(ID1,I),maxa(jj)
500   CONTINUE
C      STOP
