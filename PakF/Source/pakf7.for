...
C      IF (PENALT.LT.1.D0) THEN
C      DO I=1,IBRGT-1
C      DO J=1,IBRGT-1
C      CALL FNTERP(R(IBRGT-1,I),S(IBRGT-1,J),0,TT21,H,HP,ZVHX,ZVHY,HV2,
C     &HV3,ZVXT,ZVYT,DETJ,DETJS,X,Y,FS2,FS3,ZVXV2,ZVYV3,ZVYV2,ZVXV3,TAU,
C     &NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET,AMI,INDAMI,PENALT,PRESS,INDAX,
C     &AKT,GUSM,IUPWIN,NBREL,IIZLAZ)
C       CALL AXISYF(INDAX,DEBLJ,X,H,NDIM)
C      WDT=W(IBRGT-1,I)*W(IBRGT-1,J)*DETJ
C      DO K=1,NDIM
C      DO N=1,NDIM
...
C        IF (PENALT.LE.1.D0) IBRGT=IBRGT+1
        INDX=NGPSIL(4,JBRPS)
        INDY=NGPSIL(5,JBRPS)
...
c=======================================================================
cc Za Marka, 13 Nov 2012
c       RS2(K)=RS2(K)+(H(K))*WDT*fk1*dd(4)*tt21(2*ndim+4+k)
c=======================================================================
C       RS2(K)=RS2(K)+(H(K)*FS2)*WDT
C       RS3(K)=RS3(K)+(H(K)*FS3)*WDT
      ENDDO
c===============================================================================
      DO 206 K=1,NDIM 
C	 IF (NBREL.LT.900) GOTO 206
C       RS2(K)=RS2(K)+WDT*(H(K)*FS2)*NGPSIL(4,JBRPS)*AKT*(1.0D0)
C=====================================================================================
...
C=====================================================================================
C       RS3(K)=RS3(K)+WDT*(H(K)*FS3)*NGPSIL(5,JBRPS)*AKT
C       fs2=1.d0
...
c	  wt=-0.8d0
	  ...
	  t=0.25d0
      if (ndim.eq.8) then
c       call jact8nodeTetra (r,s,t,ck,h,tt21,nbrel,ndim,det1,pj,hp,hv1,
c     &hv2,hv3,HXU,HYU,HZU,HXV,HYV,HZV,HXW,HYW,HZW,NEL,iizlaz,ndimp)
...
C
     ...
     1DETJS,X,Y,FS2,FS3,ZVXV2,ZVYV3,ZVYV2,ZVXV3,TAU
      COMMON /TACNOS/ EPSTR,MAXIT
      COMMON /ULAZNI/ IULAZ,IIZLAZ
      COMMON /VISKOZ/ AMI,INDAMI
      COMMON /TEMPER/ BETA,TETAO,FB2,FB3
      COMMON /NUMNPT/ NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET
      COMMON /VREPER/ NPER,NTABFT
      COMMON /NDESUK/ NDES,IDPRIT,IFORM
      COMMON /PENALL/ PENALT,PRESS
      COMMON /POCETN/ IPOCU,IPOCV,IPOCP,IPOCT,POCU,POCV,POCP,POCT
      COMMON /NESTAC/ NSTAC,NKOR,DT

      CHARACTER*250NASLOV
      DIMENSION TT1(*),ZADVRE(*),CORD(3,*)
      DIMENSION NEL(NDIM+1,*),ID(4,*),NZAD(3,*)
...
C=======================================================================
	...
	DIMENSION ISPAR(9,*),ROW(*),SPARC(*),ISPARC(2,*),MAXAKC(*)
      ...
      DIMENSION TT1(*),NEL(NDIM+1,*),ID(7,*),CORD(3,*)
...
C=======================================================================
     ...
     &TABF,ITFMAX,NBREL,TIME,KKORAK,VVREME)
      ...
      DIMENSION NEL(NDIM+1,*),ID(7,*),NZAD(3,*),ITFMAX(*)
...
C      COMMON /AKIRA/ VOLT,PBALL,VSR,PBOX,DPRES
      ...
      DIMENSION X(9),Y(9),LM2(6*9),TT21(6*9),TT210(6*9),TT21A(2*9)
      DIMENSION R(3,3),S(3,3),W(3,3)
      DIMENSION H(9),ZVHX(9),ZVHY(9),HP(4)
      DIMENSION AKVV2(9,9)
      DIMENSION AKMIV2(9,9)
...
C subroutine FNTERP return to us interpolation functions...
      ...
      CALL FNTERP(R(IBRGT,I),S(IBRGT,J),1,TT21,H,HP,ZVHX,ZVHY,HV2,
     &HV3,ZVXT,ZVYT,DETJ,DETJS,X,Y,FS2,FS3,ZVXV2,ZVYV3,ZVYV2,ZVXV3,TAU,
     &NUMZAD,NPT,NDIM,MAXSIL,JEDN,NWK,NET,AMI,INDAMI,PENALT,PRESS,INDAX,
     &AKT,GUSM,IUPWIN,NBREL,IIZLAZ)
       IF (IALE.EQ.1) CALL ALEHV(TT21,TT21A,HV2,HV3,NDIM,H)
...
C=========================================================================
C==========================================================================
      SUBROUTINE NENJ3DCarreauYasuda(PJ,TT210,AMI,NDIM,IIZLAZ)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
...
C=====================================================================
	...
	GAMA=DSQRT(DII)
      ...
      AMI=AMIINF+(AMI0-AMIINF)*(1.D0+(ALAMBDA*GAMA)**A)**((AN-1.D0)/AN)
