C=========================================================================
      SUBROUTINE Mixed3D8VPTime(GNODE,ALEVO,DESNO,SILE,NEL,
...
C     1H(K)*HV2*PJ(2,N)+H(K)*HV3*PJ(3,N))*GUSM)
      
      HV10=DOT(H,TT210(1),NDIM)
      HV20=DOT(H,TT210(1+NDIM),NDIM)
      HV30=DOT(H,TT210(1+2*NDIM),NDIM)

      
      AKVV1(K,N)=AKVV1(K,N)+WDT*((H(K)*HV10*PJ(1,N)+
     1H(K)*HV20*PJ(2,N)+H(K)*HV30*PJ(3,N))*GUSM)
c=================================================================
c=================================================================
...
c       SKEF(I,J+NDIM)=AF*AJV1V2(I,J)
c       SKEF(I,J+2*NDIM)=AF*AJV1V3(I,J)
      IF (J.LE.NDIMP) THEN
c       SKEF(I,J+3*NDIM)=AF*AKV1P(I,J)
c       SKEF(I+NDIM,J+3*NDIM)=AF*AKV2P(I,J)
c       SKEF(I+2*NDIM,J+3*NDIM)=AF*AKV3P(I,J)
      ENDIF
      IF (I.LE.NDIMP) THEN
c       SKEF(I+3*NDIM,J)=AF*AKV1P(J,I)
c       SKEF(I+3*NDIM,J+NDIM)=AF*AKV2P(J,I)
c       SKEF(I+3*NDIM,J+2*NDIM)=AF*AKV3P(J,I)
C==========================================================================
C Poisson equation
C==========================================================================
	 ...
	 ENDIF
C	 IF (IPASS.EQ.4.AND.METOD.EQ.4) THEN
c        SKEF(I+3*NDIM,J+3*NDIM)=SKEF(I+3*NDIM,J+3*NDIM)+AKPP(I,J)
C	 ENDIF
	ENDIF
C==========================================================================
      ENDIF
...
c       SKEF(I+NDIM,J)=AF*AJV2V1(I,J)
c       SKEF(I+NDIM,J+NDIM)=AF*(AKMIV1(I,J)+AKVV1(I,J)+AJV2V2(I,J))
c       SKEF(I+NDIM,J+2*NDIM)=AF*AJV2V3(I,J)

c      SKEF(I+2*NDIM,J)=AF*AJV3V1(I,J)
c      SKEF(I+2*NDIM,J+NDIM)=AF*AJV3V2(I,J)
c      SKEF(I+2*NDIM,J+2*NDIM)=AF*(AKMIV1(I,J)+AKVV1(I,J)+AJV3V3(I,J))
      IF (NSTAC.EQ.0.AND.IPASS.GT.0) THEN
...
C DESNA STRANA:
      ...
      DO 285 J=1,NDIM
        JJ=J+K*NDIM
	IF (IPASS.EQ.1.and.NSTAC.eq.0) THEN
        F92(II)=F92(II)-(AKMIV1(I,J)+AKVV1(I,J))*TT210(JJ)*TIME
	ENDIF
      IF (NSTAC.EQ.0.AND.IPASS.EQ.1) THEN
        F92(II)=F92(II)+AMV2(I,J)*(TT210(JJ))
C        F92(II)=F92(II)-AMV2(I,J)*(TT21(JJ))
      ENDIF
      IF (NSTAC.EQ.0.AND.IPASS.EQ.2) THEN
        F92(II)=F92(II)+AMV2(I,J)*(TT21(JJ))
C        F92(II)=F92(II)-AMV2(I,J)*(TT21(JJ))
      ENDIF
 285  CONTINUE							      
 290  CONTINUE
...
C	IF (IPASS.NE.0.AND.IPASS.NE.4) THEN
      ...
      DO 310 I=1,NDIM
      DO 300 J=1,NDIM
      IF (J.LE.NDIMP.AND.IPASS.EQ.1) THEN
       F92(I)=F92(I)-AKV1P(I,J)*TT210(J+3*NDIM)*TIME
       F92(I+NDIM)=F92(I+NDIM)-AKV2P(I,J)*TT210(J+3*NDIM)*TIME
       F92(I+2*NDIM)=F92(I+2*NDIM)-AKV3P(I,J)*TT210(J+3*NDIM)*TIME
      ENDIF
      IF (J.LE.NDIMP.AND.IPASS.EQ.2) THEN
       F92(I)=F92(I)-AKV1P(I,J)*(TT21(J+3*NDIM)-TT210(J+3*NDIM))*TIME
       F92(I+NDIM)=F92(I+NDIM)-AKV2P(I,J)*
     &(TT21(J+3*NDIM)-TT210(J+3*NDIM))*TIME
       F92(I+2*NDIM)=F92(I+2*NDIM)-AKV3P(I,J)*  
     &(TT21(J+3*NDIM)-TT210(J+3*NDIM))*TIME
      ENDIF
      IF (I.LE.NDIMP) THEN
C       F92(I+3*NDIM)=F92(I+3*NDIM)-AKV1P(J,I)*TT21(J)
C       F92(I+3*NDIM)=F92(I+3*NDIM)-AKV2P(J,I)*TT21(J+NDIM)
C       F92(I+3*NDIM)=F92(I+3*NDIM)-AKV3P(J,I)*TT21(J+2*NDIM)
      ENDIF
 300  CONTINUE
 310  CONTINUE
C      ENDIF
...
C        F92(I+3*NDIM)=F92(I+3*NDIM)-AKPP(I,J)*TT21(J+3*NDIM)
        F92(I+3*NDIM)=F92(I+3*NDIM)+AKPP(I,J)*TT210(J+3*NDIM)
C        F92(I+3*NDIM)=F92(I+3*NDIM)-AKPP(I,J)*TT21(J+3*NDIM)
       ENDIF
C==========================================================================
 301  CONTINUE
...
c     &(AKVV1P(I,J)+AKMIV1P(I,J))*TT21(J+4*NDIM)
     &(AKV1P(I,J))*TT21(J+4*NDIM)
       GNODE(2,2,NODE)=GNODE(2,2,NODE)+
c     &(AKVV2P(I,J)+AKMIV2P(I,J))*TT21(J+4*NDIM)
     &(AKV2P(I,J))*TT21(J+4*NDIM)
       GNODE(2,3,NODE)=GNODE(2,3,NODE)+
c     &(AKVV3P(I,J)+AKMIV3P(I,J))*TT21(J+4*NDIM)
     &(AKV3P(I,J))*TT21(J+4*NDIM)
...
C=======================================================================
C==========================================================================
      SUBROUTINE FILLN2Pt(GNODE,SILE,ID,NPT,IPASS)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
