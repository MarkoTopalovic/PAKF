C=========================================================================
      SUBROUTINE RACU3Dtetra_seg10_4(GNODE,ALEVO,DESNO,SILE,NEL,
     ...
     &VMESH,IALE,AF,NJUTRA,ISYMMS,ITER,IPASS,METOD,NUMPASS,KKORAK)
...
c      AJV3V3(K,N)=AJV3V3(K,N)+H(K)*HZW*H(N)*GUSM*WDT
       ...
       AKV1PT(K,N)=AKV1PT(K,N)+WDT*(PJ(1,K)*Hp(N))
       AKV2PT(K,N)=AKV2PT(K,N)+WDT*(PJ(2,K)*Hp(N))
       AKV3PT(K,N)=AKV3PT(K,N)+WDT*(PJ(3,K)*Hp(N))
      ENDIF
 164  CONTINUE
...
c     &AMI,NDIM,ISRPS,IIZLAZ,IUPWIN,NEL,SHEAR,ZVXT,ZVYT,ZVZT,DTEMPDN)
      ...
      IF (N.LE.NDIMP.AND.PENALT.LT.1.D0) THEN
       AKV1P(K,N)=AKV1P(K,N)+WDT*(PJP(1,N)*H(K))
       AKV2P(K,N)=AKV2P(K,N)+WDT*(PJP(2,N)*H(K))
       AKV3P(K,N)=AKV3P(K,N)+WDT*(PJP(3,N)*H(K))
	ENDIF
...
C     &AKV3P(K1,N)*(1.D0/AKIIZ(K1,K1))*AKV3P(K1,K)
	AKPP(N,K)=AKPP(N,K)+
     &AKV1Pt(K1,N)*(1.D0/AKIIX(K1,K1))*AKV1P(K1,K)+
     &AKV2Pt(K1,N)*(1.D0/AKIIY(K1,K1))*AKV2P(K1,K)+
     &AKV3Pt(K1,N)*(1.D0/AKIIZ(K1,K1))*AKV3P(K1,K)
	   ENDDO
	  ENDDO
	 ENDDO
	ENDIF
...
C==========================================================================
	...
	II=I+3*NDIM
	DO J=1,NDIM
        F92(II)=F92(II)-AKV1Pt(J,I)*(1.D0/AKIIX(J,J))*RP1(J)
        F92(II)=F92(II)-AKV2Pt(J,I)*(1.D0/AKIIY(J,J))*RP2(J)
        F92(II)=F92(II)-AKV3Pt(J,I)*(1.D0/AKIIZ(J,J))*RP3(J)
	ENDDO
	ENDDO
...
c       ALAMBDA=0.D0
c	 IF (NUMPASS.EQ.4) ALAMBDA=1.0D0
c       F92(I+3*NDIM)=F92(I+3*NDIM)-AKV1Pt(J,I)*TT21(J)*ALAMBDA
c       F92(I+3*NDIM)=F92(I+3*NDIM)-AKV2Pt(J,I)*TT21(J+NDIM)*ALAMBDA
c       F92(I+3*NDIM)=F92(I+3*NDIM)-AKV3Pt(J,I)*TT21(J+2*NDIM)*ALAMBDA
...
C==========================================================================
	...
	IF (IPASS.EQ.4.AND.METOD.EQ.4) THEN
      ...
      DO I=1,NDIMP
      DO J=1,NDIM
       F92(I+3*NDIM)=F92(I+3*NDIM)-AKV1Pt(J,I)*TT21(J)*1.d-2
       F92(I+3*NDIM)=F92(I+3*NDIM)-AKV2Pt(J,I)*TT21(J+NDIM)*1.d-2
       F92(I+3*NDIM)=F92(I+3*NDIM)-AKV3Pt(J,I)*TT21(J+2*NDIM)*1.d-2

C       IF (J.LE.NDIMP.and.kkorak.eq.1) THEN
       IF (J.LE.NDIMP) THEN
c       F92(I+3*NDIM)=F92(I+3*NDIM)-
C     &AKPP(I,J)*(TT21(J+3*NDIM)-TT210(J+3*NDIM))
c     &AKPP(I,J)*(TT21(J+3*NDIM))
       ENDIF
...
C	DO K=1,NDIM
	IF (LM2(I).NE.0) THEN
     ...
     &(AKV1P(I,J))*(1.D0/AKIIX(I,I))*TT21(J+3*NDIM+NDIMP)
	ENDIF
	IF (LM2(I+NDIM).NE.0) THEN
       GNODE(2,2,NODE)=GNODE(2,2,NODE)+
     &(AKV2P(I,J))*(1.D0/AKIIY(I,I))*TT21(J+3*NDIM+NDIMP)
	ENDIF
	IF (LM2(I+2*NDIM).NE.0) THEN
       GNODE(2,3,NODE)=GNODE(2,3,NODE)+
     &(AKV3P(I,J))*(1.D0/AKIIZ(I,I))*TT21(J+3*NDIM+NDIMP)
	ENDIF
