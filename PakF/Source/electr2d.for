C=========================================================================
      SUBROUTINE ELECTR2D(GNODE,NEL,ID,CORD,SKEF,
...
C NBREL is counter of elements
      ...
      CALL JAYADEEP(GUSM,CC,AKT,SIGMA,NBREL,NET)
...
C     &ZVYV3,ZVYV2,ZVXV3,ZVHX,ZVHY)
     ...
     1*GUSM*CC)
      ...
      AFIFI2(K,N)=AFIFI2(K,N)-WDT*((H(K)*HFI2*ZVHX(N)+
     1H(K)*HFI3*ZVHY(N))*SIGMA)
...
C====================================================================
C ADDITIONAL TERM, ELECTRICAL FIELD
       SKEF(I+2*NDIM+4,J+3*NDIM+4)=AF*(2.D0*AFIFI2(I,J))
C====================================================================
...
c Electrical field
      ...
      F31(I+2*NDIM+4)=F31(I+2*NDIM+4)-AFIFI2(I,J)*TT21(J+3*NDIM+4)
c======================================================
...
C=======================================================================
C=========================================================================
      SUBROUTINE JAYADEEP(GUSM,CC,AKT,SIGMA,NBREL,NET)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
       
C ELECTRODE      
	IF (NBREL.GE.1.AND.NBREL.LE.360) THEN
	 GUSM=2.15D-5
	 CC=132.D0
	 SIGMA=4.D3 
       AKT=7.1D-2
...
C SALINE
c	IF (NBREL.GE.361.AND.NBREL.LE.630) THEN
	IF (NBREL.GE.571.AND.NBREL.LE.600) THEN
	 GUSM=0.98D-6
	 CC=4184.D0
	 SIGMA=1.35D-3
       AKT=6.28D-4
...
C MYOCARDIUM      
	IF ((NBREL.GE.361.AND.NBREL.LE.570).OR.
     &(NBREL.GE.601.AND.NBREL.LE.NET)) THEN
	 GUSM=1.2D-6
	 CC=3200.D0
	 SIGMA=6.1D-4
       AKT=7.0D-4
