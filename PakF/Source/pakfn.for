...
C
C======================================================================
      SUBROUTINE TGRMATF(NTABK,NUMMAT,NETIP,II,NDIM,NASLOV,GAMA,NTABC)
...
C      DIMENSION GAMA(*),TABK(2,INTABK,*),TABC(2,NUMMAT,*)
      DIMENSION GAMA(*),NTABK(2,NUMMAT,*),NTABC(2,NUMMAT,*)
C
      IND=-1
      ITYP=100
	VERSION=4.4
...
C======================================================================
      ...
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
C
      DIMENSION GNODE(2,7,*),FSP(7),IN(8)
C
      FSP(3)=0.
C
      JEDAN=1
...
C------------------------------------------------------------------------
      ...
      IF(IN(1).EQ.22.AND.IP.EQ.1) WRITE(II,3022) 
C------------------------------------------------------------------------
...
C------------------------------------------------------------------------
      IF(IN(1).EQ.21) WRITE(II,1000) NULA,NULA,M6,M7
      IF(IN(1).EQ.22) WRITE(II,1000) NULA,NULA,M6,M7
C------------------------------------------------------------------------
...
C------------------------------------------------
	   IF(IP.EQ.1.AND.IN(1).EQ.21) VAL=FSP(6)
	   IF(IP.EQ.1.AND.IN(1).EQ.22) VAL=FSP(7)           
C------------------------------------------------
         IF(IP.EQ.1.AND.IN(1).GT.40)
...
C-------------------------------
 3021 FORMAT('TURBULENT K')
 3022 FORMAT('TURBULENT OMEGA')
C-------------------------------
 ...
 5000 FORMAT(I9,',',3(1PE12.4,','))
      END
C=======================================================================
C======================================================================
      SUBROUTINE STAU10F(PRES,NPP,II,IND,KOR)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
C     ZA STAMPANJE PRITISAKA
